...
from utils.network_utils import model_load, measure_efficiency
import tqdm
...
def main(args):
    ...
    model = FlowNetS(args, tasks, args.block, args.nr_blocks).to('cuda')
    params, fps = measure_efficiency(args)
    print(params, fps)

    model = torch.nn.DataParallel(model).to('cuda')
    # Load checkpoint
    load_model_path = os.path.join(args.out, 'ckpt_110.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', dest='model_path', help='Set model path', default='/media/efklidis/4TB/RESULTS-ECAI/flownet', type=str)
    parser.add_argument('--out', dest='out', help='Set output path', default='/media/efklidis/4TB/RESULTS-ECAI/flownet', type=str)
