...
def save_outputs(args, seq_name, name, output_dict):
    ...
    cv2.imwrite('{}/{}'.format(os.path.join(args.out, seq_name, 'flows'), name[:-4] + '.png'),
                flow_to_image(output_dict['flow'][1])[0].permute(1, 2, 0).cpu().numpy())
...
"ed351457-849e-4e27-9c6e-448718e9c4ac_GT_104379.png"]
    ...
    for seq_idx, seq in enumerate(dataloader['test']):
        ...
        for frame in tqdm(range(args.prev_frames, len(seq['meta']['paths']))):
            ...
            results.write(
                '\nFrame {}, PSNR: {:.3f},  SSIM: {:.3f}, EPE: {:.3f}, EPE_m: {:.3f}, EPE_l: {:.3f} IoU: {:.3f} DiCE: {:.3f} IoU_HL: - DiCE_HL: -\n'.format(
                    name, metrics_values['psnr'], metrics_values['ssim'],
                    metrics_values['EPE'], metrics_values['EPE_med'], metrics_values['EPE_low'],
...
def main(args):
    ...
    # Load checkpoint
    resume_path = os.path.join(args.out, 'ckpt_{}.pth'.format(92))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', dest='out', help='Set output path', default='/media/efklidis/4TB/RESULTS-ECAI/mostnet-fw/', type=str)
