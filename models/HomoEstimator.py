...
class HomoEstimator4(nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, padding=1, bias=False),
                                         nn.BatchNorm2d(256),
        ...
        self.head = torch.nn.Sequential(nn.Conv2d(256, 32, kernel_size=3, padding=1),
                                        nn.Dropout(0.25),
                                        nn.Conv2d(32, 2, kernel_size=3, padding=1))


    def forward(self, m1, m2):
        x = self.conv1(torch.cat([m1, m2], 1)) # 260 x 80 x 106 -> 256 x 40 x 53
        x = self.conv2(x)  # 256 x 40 x 53 -> 256 x 20 x 26
